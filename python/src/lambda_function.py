...
# This is a Square Root Calculator Alexa Skill.
# The skill serves as a simple sample on how to get
# and use user input.
...
skill_name = "Square Root"
help_text = ("Please give me a number to work on.")
...
def launch_request_handler(handler_input):
    ...
    speech = "Welcome to the Square Root calculator."
...
@sb.request_handler(can_handle_func=is_intent_name("SquareRootIntent"))
def square_root_handler(handler_input):
    # type: (HandlerInput) -> Response
    slots = handler_input.request_envelope.request.intent.slots

    if number_slot in slots:
        given_number = slots[number_slot].value
        sqrt_number = math.sqrt(Decimal(given_number))
        rnd_number = round(sqrt_number,2)
        speech = ("The square root of {} is {}".format(given_number, rnd_number))
...
def fallback_handler(handler_input):
    ...
    speech = (
        ...
        "This skill can tell you the square root of a number.".format(skill_name))
    reprompt = ("Please say a number you'd like the square root for")
    handler_input.response_builder.speak(speech).ask(reprompt)
    return handler_input.response_builder.response
